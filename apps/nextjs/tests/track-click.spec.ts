...
test('should set cq_id cookie when ?cq_id= is present', async ({ page }) => {
  const clickId = 'kVrpHmuNu8Q9VpBG';
  await page.goto(`/?cq_id=${clickId}`);
  ...
  await expect(async () => {
    ...
    expect(codeqrIdCookie?.value).toBe(clickId);
  }).toPass({ timeout: 5000 });
});
...
test('should fire /track/click and set cookies when ?via= is present', async ({
  ...
  await expect(async () => {
    ...
    // Check if the cq_id cookie is set
    const codeqrIdCookie = cookies.find((cookie) => cookie.name === 'cq_id');
    expect(codeqrIdCookie).toBeDefined();
    expect(codeqrIdCookie?.value).toBe(responseData.clickId);

    // Check if the codeqr_partner_data cookie is set
    const partnerDataCookie = cookies.find(
      (cookie) => cookie.name === 'codeqr_partner_data',
    );
